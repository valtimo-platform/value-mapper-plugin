...
export class ValueMapperListComponent implements OnInit {
    ...
    private reloadTemplateList(): void {
        this.loading$.next(true);
        this._caseDefinitionName$.pipe(
            switchMap(caseDefinitionName => this.valueMapperService.getValueMapperDefinitions()),
    ...
    private setSelectedTemplateKeys(): void {
        this.selectedRowKeys$.next(this.carbonList.selectedItems.map((mapper: ValueMapperListItem) => mapper.key));
    }
}
